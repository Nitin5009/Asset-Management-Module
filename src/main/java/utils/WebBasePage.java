...
public class WebBasePage extends WaitStatement {
    ...
    public void ClickByView(By by, String name, int time) {
        ...
        if (element != null) {
            try {
                ...
                staticWait(500);
                element.click();
                getTest().log(LogStatus.PASS, name + " is scrolled to visible and clicked");
                logger.info(name + " is scrolled to visible and clicked");
            } catch (Exception e) {
    ...
    public void takeScreenshot(String name) {
        String imageName =  name + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
        String imagePath = System.getProperty("user.dir") + "\\reports\\" +imageName;
        ...
        getTest().log(LogStatus.INFO, getTest().addScreenCapture("./" + imageName + ".png"));
    }
    ...
    public void waitTillNewFile(String path, int previousFileCount) {
        try {
            staticWait(5000);
            int timeCount = 1;
            for (timeCount = 1; timeCount < 120; timeCount++) {
    ...
    public boolean alertAccept(String message){
        try {
            String alertText = driver.switchTo().alert().getText();
            driver.switchTo().alert().accept();
            getTest().log(LogStatus.PASS, "Alert present with " + message);
            logger.info("Alert present with " + message);
            return alertText.equals(message);
        }catch (Exception e){
            getTest().log(LogStatus.FAIL, "Alert present with " + message);
            return false;
        }
    }
    public  void countOfDropdownValues(By by, String name, int time){
        staticWait(200);
        try {
            WebElement element = findElementVisibility(by, time);
            ...
            for (int elementSize = 0; elementSize < itemSize; elementSize++) {
                String optionsValue = elementCount.get(elementSize).getText();
                getTest().log(LogStatus.PASS,name+" of" +optionsValue+" is displayed");
                logger.info(optionsValue+" status is displayed");
            }
        } catch (Exception e) {
            getTest().log(LogStatus.FAIL, name+" is not displayed");
